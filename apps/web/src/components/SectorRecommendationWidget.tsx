...
export function SectorRecommendationWidget({ style }: SectorRecommendationWidgetProps) {
  ...
  return (
    <div className="space-y-3">
      <label htmlFor="sector-select" className="block text-sm text-slate-400">
        Sector
      </label>
      <select
        id="sector-select"
      ...
      <ul className="space-y-3">
        {companies.map((company) => (
          <li key={company.ticker} className="border border-slate-800 rounded-lg p-3">
            <div className="flex justify-between items-baseline">
              <span className="font-medium text-slate-200">{company.ticker}</span>
