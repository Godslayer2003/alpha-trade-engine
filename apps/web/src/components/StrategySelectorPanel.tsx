      ...
      {!user ? (
        ...
        <div className="border-t border-slate-800 pt-3 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm text-slate-400">My strategies</span>
            <button
              onClick={() => setShowForm((v) => !v)}
              className="text-xs px-2.5 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-300 hover:bg-slate-700"
            >
          ...
          {showForm && (
            <div className="space-y-2 border border-slate-800 rounded-lg p-3">
              <input
                value={form.name}
                onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                placeholder="Strategy name (e.g. Tech Momentum)"
                className="w-full rounded-lg bg-slate-800 border border-slate-700 text-slate-100 px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
              <select
                value={form.style}
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-20 rounded-lg bg-slate-800 border border-slate-700 text-slate-100 px-2.5 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
          ...
          {strategies.length > 0 && (
            <ul className="space-y-2">
              {strategies.map((s) => (
                <li key={s.id} className="border border-slate-800 rounded-lg p-2.5 text-xs">
                  <div className="flex justify-between items-baseline">
                    <span className="font-medium text-slate-200">{s.name}</span>
                    <span className="text-emerald-400">{STYLE_LABELS[s.style as InvestmentStyle]}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <button
                      ...
                      className="px-2 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-300 hover:bg-slate-700"
                    >
                      {s.isActive ? 'Deactivate' : 'Activate'}
                    </button>
                    <button
                      onClick={() => handleDelete(s.id)}
                      className="px-2 py-1 rounded-full bg-slate-800 border border-slate-700 text-rose-400 hover:bg-slate-700"
                    >
                      Delete
                    </button>
                  </div>
